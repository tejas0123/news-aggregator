...
public class UpdateServerDetailsHandler implements PutRequestsHandler {
    @Override
    ...
    public APIResponse sendAPIRequest(Object data) {
        ServerDetails updatedDetails = (ServerDetails) data;
        URI updateServerUri = buildUri("/api/v1/admin/server");
        Map<String, String> headers = Map.of(Constants.AUTHORIZATION, Constants.BEARER + JwtUtil.getToken().get());

        HttpRequest request = buildHttpRequest(updatedDetails, updateServerUri, headers);
        return sendHttpRequest(request,  new TypeReference<Response<Void>>() {});
    }
}
