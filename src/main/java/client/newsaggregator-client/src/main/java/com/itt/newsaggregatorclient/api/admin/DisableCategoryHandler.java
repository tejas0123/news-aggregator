...
import com.itt.newsaggregatorclient.api.PatchRequestsHandler;
...
public class DisableCategoryHandler implements PatchRequestsHandler {
    @Override
    ...
    public APIResponse sendAPIRequest(Object data) {
        URI disableCategoryUri = buildUri("/api/v1/admin/categories");
        Map<String, String> headers = Map.of(Constants.AUTHORIZATION, Constants.BEARER + JwtUtil.getToken().get());
        Set<Integer> categoryIds = (Set<Integer>) data;

        HttpRequest httpRequest = buildHttpRequest(categoryIds, disableCategoryUri, headers);
