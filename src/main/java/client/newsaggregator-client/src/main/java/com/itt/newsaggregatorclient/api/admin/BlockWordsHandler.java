...
public class BlockWordsHandler implements PostRequestsHandler {
    @Override
    ...
    public APIResponse sendAPIRequest(Object data) {
        URI blockWordsUri = buildUri("/api/v1/admin/categories/words/blocked");
        Map<String, String> headers = Map.of(Constants.AUTHORIZATION, Constants.BEARER + JwtUtil.getToken().get());
        Set<String> wordsToBlock = (Set<String>) data;
        HttpRequest httpRequest = buildHttpRequest(wordsToBlock, blockWordsUri, headers);
